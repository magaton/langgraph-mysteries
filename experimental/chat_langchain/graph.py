...
from experimental.utils import format_docs, load_chat_model
from experimental.types import BaseChatConfig
from experimental.chat_langchain.configuration import AgentConfiguration
from experimental.chat_langchain.researcher_graph.graph import graph as researcher_graph
from experimental.chat_langchain.state import AgentState, InputState, Router
...
async def analyze_and_route_query(
    ...
    if state.router and state.router["logic"]:
        print(f"Already set router to {state.router['logic']}")
        return {"router": state.router}
    else:
         print(f"Call LLM to figure out Routing")
    configuration = AgentConfiguration.from_runnable_config(config)
    print(f"query_model: {configuration.query_model.model_dump_json()}, type: {type(configuration.query_model)}")
...
def route_query(
    ...
    print(f"route_query -> type: {_type}")
    if _type == "langchain":
...
def get_graph()-> CompiledStateGraph:
    ...
    workflow.add_node(conduct_research)
    workflow.add_node(respond)
    workflow.add_edge(START, "analyze_and_route_query")
    workflow.add_edge("create_research_plan", "conduct_research")
    workflow.add_conditional_edges("conduct_research", check_finished)
    workflow.add_edge("ask_for_more_info", END)
    workflow.add_edge("respond_to_general_query", END)
    workflow.add_edge("respond", END)
    graph = workflow.compile().with_config(config=RunnableConfig(configurable=AgentConfiguration().model_dump()))
